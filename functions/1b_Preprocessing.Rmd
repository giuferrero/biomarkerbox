...
write.table(paste0(outhead, outnorm), "Preprocessing_Table_Normal_distribution.txt", quote=F, row.names=F, col.names=F, sep="\t")
```
`r outnorm`
...
# Write of the output file
write.table(out, "Preprocessing_Table_Outlier_analysis.txt", quote=F, row.names=F, col.names=F, sep="\t")
```
`r outoutlier`
...
for(i in 1:length(outlier_attr)){
  ...
  pdf(paste0("Preprocessing_Plot_Outlier_analysis_histogram_", var, ".pdf"))
  print(p)
  dev.off()
}}
```
