...
dir.create("03_Count_Data_Analysis")
...
In this document are reported the result of the DESeq2 differential expression analysis applied on the attributes of the **`r basename(params$cdata)`** file. The analysis is performed considering the **`r length(cov_class)`** classes of the  **`r refcov`** attribute. 
...
if(sum(covord %in% phenoData[,refcov]) == length(covord)){
  cat(paste0("The ", paste0(covtomodel, collapse = ", "), " attribute(s) is/are included in the sample classes.\n\n"), sep="")}else{
  stop(paste0("One or more indicated classe is not included in the sample classes.\n\n"))}
```

Attributes with zero counts in each sample were filtered out. A total of **`r nrow(countData)`** attributes were analyzed.
...
# Write the normalized count table and the table reporting the average and the median read counts
 write.table(data.frame("ID"=rownames(norm_counts), norm_counts), "DESeq_Table_Normalized_count_table.txt", sep="\t", quote=FALSE, row.names=F)
 write.table(data.frame("ID"=rownames(out), out), "DESeq_Table_Median_average_normalized_counts.txt", sep="\t", quote=FALSE, row.names=F)
...
for(i in 1:ncol(comparisons)){
  ...
  write.table(data.frame("ID"= rownames(res_sub), res_sub), file= paste0("DESeq_Table_DE_results_", cov1, "_vs_", cov2, ".txt"), sep="\t", quote=FALSE, row.names=F)
...
For each comparison a file names **DESeq_Table_DE_results_(class1)_vs_(class2).txt** was generated in which class1 and class2 correspond to the sample classes compared. This file reports the result of the differential expression analysis for each analysed feature.

For each comparison a file named **DESeq_Table_DE_results_(class1)_vs_(class2)_significant.txt** was generated in which class1 and class2 correspond to the sample classes compared. This file reports the significant results of the differential expression analysis associated with absolute log2FC greater than **`r params$logfcthreshold`** and adjusted p-value lower than **`r params$pvalthreshold`**. For each comparison, a Volcano Plot showing the relationship between the log2FC and adjusted p-value computed for each analysed attribute is provided.
...
write.table(de, file= paste0("DESeq_Table_DE_results_", covid[1], "_vs_", covid[2], "_significant.txt"), sep="\t", quote=FALSE, row.names=F)

cat("The differential expression analysis between **", covid[1], "** and **", covid[2], "** detected **", nrow(de), "** significanlty differentially expressed feature(s).\n\n", sep = "")
...
pdf(paste0("DESeq_Plot_Volcano_plot_", covid[1], "_vs_", covid[2], ".pdf"))
print(p)
dev.off()
...
totest[,1] <- factor(totest[,1], levels = covord)
...
write.table(complete, paste0("DESeq_Table_Merged_DESeq2_results.txt"), quote=F, col.names=NA, sep="\t")
```
...
pdf(paste0("DESeq_Plot_Upset_DE_result_overlap.pdf"))
print(p)
dev.off()  
...
out <- "The following data report the list of attributes overlapped among different comparisons.\n\n"

for(i in 1:length(resoverlap)){
names(resoverlap)[i] <- paste(names(res2)[combos[[i]]], collapse=", ")
cat(out, ": ", names(resoverlap)[i], "\n", paste0(resoverlap[[i]], collapse=", "), "\n\n")
out <- paste0(out, names(resoverlap)[i], "\n")
out <- paste0(out, paste(resoverlap[[i]], collapse=","), "\n\n")
}

cat("The file named **DESeq_Table_Overlapped_Results.txt** reporting these results was generated. \n\n")

write.table(out, "DESeq_Table_Overlapped_Results.txt", quote=F, row.names=F, col.names=F)
...
* DESeq_Table_DE_results_(class1)_vs_(class2).txt (class1, class2 = sample classes compared)
* DESeq_Table_DE_results_(class1)_vs_(class2)_significant.txt (class1, class2 = sample classes compared)
* DESeq_Table_Merged_DESeq2_results.txt
* DESeq_Table_Overlapped_Results.txt
* DESeq_Plot_Volcano_plot_(class1)_vs_(class2).pdf (class1, class2 = sample classes compared)
* DESeq_Plot_analysis_boxplot_(attribute).pdf (attribute = specific attribute plotted)
* DESeq_Plot_Upset_DE_result_overlap.pdf

All the output file were generated in the folder **03_Count_Data_Analysis**