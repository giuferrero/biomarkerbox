...
boxp <- function(dat, covid, refcov, inlinep=FALSE){
  ...
  printout(paste0(outf, "DESeq_Plot_analysis_boxplot_", covid, ".pdf"), p)
...
if(sum(covtomodel %in% names(phenoData)) == length(covtomodel)){
  ...
  cat(paste0("The attribute(s) ", paste0(setdiff(covtomodel, names(phenoData)), collapse = ", "), " is/are not included in the sample attributes.\n\n"), sep="")
  stop("Exiting")}

# Check order of attributes
if(sum(covord %in% phenoData[,refcov]) == length(covord)){
  cat(paste0("The ", paste0(covtomodel, collapse = ", "), " attribute(s) is/are included in the sample attributes.\n\n"), sep="")}else{
  cat(paste0("The  ", paste0(setdiff(covord, phenoData[,refcov]), collapse = ", "), " is/are not included in the sample attributes.\n\n"))
  stop("Exiting")}
```
...
For each comparison a file names **DESeq_Table_DE_results_(class1)_vs_(class2).txt** was generated in which cov1 and cov2 correspond to the sample classes compared. This file reports the result of the differential expression analysis for each analysed feature.
...
p <- EnhancedVolcano(listout[[1]], lab=row.names(listout[[1]]), x = 'log2FoldChange', y = 'padj', pCutoff = 0.001, FCcutoff = 1, ylab = "-log10(Adjusted p-value)", legend=c('NS','log2(FC)','Adj. p-value', 'Adj. p-value & log2(FC)'))
...
totest <- data.frame(Class=phenoData[,refcov], value=t(norm[covid,]), check.names=F)
totest[,1] <- factor(totest[,1], levels = covord)
totest[,2] <- log10(totest[,2]+1)

# Boxplot for the significant differentially expressed attributes  
 for(j in 1:nrow(de)){
  
  covid <- as.character(de$ID[j])
  totest <- data.frame(Class=phenoData[,refcov], value=t(norm[covid,]), check.names=F)
  totest[,1] <- factor(totest[,1], levels = covord)
  totest[,2] <- log10(totest[,2]+1)
  boxp(totest, covid, refcov, inlinep=FALSE)}
