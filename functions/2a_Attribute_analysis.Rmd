...
if(sum(covord %in% phenoData[,refcov]) == length(covord)){
  cat(paste0("All the indicated sample classes are included.\n\n"), sep="")}else{
  cat(paste0("The  ", paste0(setdiff(covord, phenoData[,refcov]), collapse = ", "), " is/are not included in the sample attributes.\n\n"))
  stop("One or more classes is/are not included in the samples classes")}
```
...
# Output writing
write.table(res, "Attribute_analysis_Table_Differential_attribute_among_groups.tsv", sep="\t", quote=F, row.names=FALSE)
...
stackedbar <- function(dat, covid, inlinep=FALSE){
  ...
  printout(paste0("Attribute_analysis_Plot_Stacked_barplot_", covid, ".pdf"), p)}
...
groupedbar <- function(dat, covid, inlinep=FALSE){
  ...
  if(inlinep==TRUE){print(p)}
  printout(paste0("Attribute_analysis_Plot_Grouped_barplot_", covid, ".pdf"), p)}
...
boxp <- function(dat, covid, refcov, inlinep=FALSE){
  ...
  if(inlinep==TRUE){print(p)}
  printout(paste0("Attribute_analysis_Plot_Boxplot_", covid, ".pdf"), p)}
...
checkandplot <- function(dat, n=1, inline=FALSE){
  ...
  totest[,refcov] <- factor(totest[,refcov], levels = covord)
...
for(i in 1:ncol(comparisons)){
  ...
  # Output writing
  write.table(res, paste0("Attribute_analysis_Table_Different_attribues_", cov1, "_vs_", cov2, ".tsv"), sep="\t", quote=F, row.names=FALSE)
...
All the output file were generated in the folder **02_Sample_Data_Analysis**